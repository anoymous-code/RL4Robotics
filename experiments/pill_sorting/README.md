...
ALOHA 双臂 MuJoCo 分药场景，任务形态经四代演进：

- **v4 轮式移动工作站（主线）**：双臂、载物台面、盒 A/B 全部装在差速底盘车体上
  （平面三关节 x/y/yaw + 位置伺服近似，轮-地接触不建模）。机器人驶离充电桩、
  停到工作位，再执行 v3 的全闭环分药；行驶中药板竖插盒 A 随车（纯接触无固定）。
- **v3 全闭环（并入 v4）**：药板竖插在装有多块铝塑板的**盒 A**（3 槽插板架）中。左手真实
...
| 文件 | 说明 |
|---|---|
| `run_full_demo.py` | **v4 主演示**：导航 → 取板 → 撕剪 ×2 → 入盒 B → 放回盒 A 全流程编排 + 录像 + 载荷曲线 |
| `tear_scene.py` | v4 场景构建：车载家具（台面/轮子/盒 A/盒 B）+ 自由体药板（8 格 + 12 条焊接易撕线）+ 底盘工具函数（直接运行可探测/渲染静帧） |
| `aloha_tear.xml` / `scene_tear.xml` | v4 机器人模型（双臂 + 车体）与房间场景包装，由 `gen_tear_model.py` 从 v1 文件生成 |
...
# 产出: docs/assets/videos/pill_full_v4_mobile_multicam.mp4 与 docs/assets/images/pill_full_v4_load.png
...
- **底盘 = 平面三关节 + 位置伺服（v4）**：世界系 slide x/y + 车体 hinge yaw，脚本按差速车
  编排（原地转向 → 沿车头直行 → 回正）；轮-地接触不建模，轮子为纯视觉几何。车体大质量
  （~60 kg）+ 关节阻尼使操作反力不会推动底盘。台面顶面在车体系 z=0（与 v3 桌面同高），
  车停到原点后车体系与世界系重合，v3 全部操作坐标直接复用。
- **非固定载荷运输**：药板竖插盒 A 槽中随车行驶，纯靠槽壁接触约束；min-jerk 位置轨迹
  限制加减速（峰值 ~0.5 m/s²），1.4 m 行驶后板在槽内漂移 0.1 mm。

...
## 4. 开源轮式双臂人形：平台选型（2026-07-18 更新）

要在"整机形态"上做 IL/VLA + RL，有两个靠谱的开源模型：

| 平台 | 来源/许可 | 形态 | 生态 |
|---|---|---|---|
| **PAL TIAGo++**（`pal_tiago_dual`） | MuJoCo Menagerie / Apache-2.0 | 全向轮底盘 + 升降躯干 + 双 7DOF 臂 + 平行爪（21 DOF） | 模型质量高，直接可用；真机是欧洲实验室标配 |
| **Galaxea R1 / R1 Pro / R1 Lite** | OpenGalaxea（官方开源 SDK + URDF） | 全向底盘 + 双 7DOF 臂（26 DOF），VR 遥操作 | **GalaxeaManipSim**：官方仿真基准 + 专家演示管线 + LeRobot 数据转换 + Diffusion Policy 基线——与本课路线完全同构的参考实现 |

我们已把 TIAGo++ 集成进分药场景做了可行性探测（`experiments/pill_sorting/probe_tiago.py`）：
模型编译 `nq=32, nu=25`，执行器分组为 4 轮速（全向底盘）+ 躯干升降 + 头部 2 +
双臂位置伺服 ×14 + 双爪 ×4——与我们现有控制框架（位置伺服 + 关节插值 + DLS IK）无缝
对接；升躯干后双臂工作域覆盖 0.75 m 标准桌面。DLS IK 本来就支持任意关节数，
迁移只需换关节名清单并加躯干升降这一冗余自由度。

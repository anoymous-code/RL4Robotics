...
!!! info "任务形态演进（2026-07-17 更新）"
    v0/v1 探索的是"按压取药"（把药片压出泡罩）。结合真实分药流程反馈，主线任务已升级为 **v2 撕剪分装**：一板 8 片，沿易撕线撕下包含目标药片的单格（药片保持密封），连板带药投入托盘——更卫生、可追溯，也是照护机构的主流做法。按压取药保留为力控基本功练习。
...
- [ ] v2 目标格随机化（8 格任选 + 板位姿扰动），测脚本策略鲁棒边界
- [ ] 把 v2 场景改造成 Gymnasium 环境（域随机化 + 奖励设计）
